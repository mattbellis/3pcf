...
#define THETA_NBINS 10 // Setting this to 32 saves us 10% in time.
#define THETA_LO -0.166
#define THETA_HI 0.944
                ...
                //for(k=j+1; k<zmax; k++)
            {
                ...
                for (int n=0;n<3;n++)
                {
                    ...
                    if (n==0) {
                        s = shortest;
                        q = shortest/middle;
                        theta0 = (longest-middle)/shortest; //(acosf((shortest2 + middle2 - longest2)/(2*shortest*middle)))*INVPI;
